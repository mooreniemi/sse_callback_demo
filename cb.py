...
def ssec():
  # pass over the args, get back listening address
    listen_to = requests.get("http://localhost:8080/q",
        params=request.args)
...
# this technique doesn't use sse but lacks timeouts
@app.route('/req')
def req():
  q = "meow"
  ...
  address = ('localhost', get_open_port())
  listener = Listener(address, authkey=b'secret password')

  try:
    print(f'sending {address}')
    requests.post("http://localhost:8080/respond",
        params=request.args,
        json={'host': address[0], 'port': address[1]},
        timeout=0.00001)
  ...
  try:
    while listening:
      while conn.poll(): # if for only 1 message
        msg = conn.recv() # blocks...
        print(msg)
        if type(msg) is dict:
          response.update(msg)
        if msg == 'close':
